...
const Navicons = () => {
  ...
  return (
    <div className='flex items-center gap-4 xl:gap-6'>
      ...
      {isProfileOpen &&
        <div className='absolute p-4 rounded-md top12 left-0 text-sm shadow-[0_3px_10px_rgb(0,0,0,0.2)] z-20'>
      ...
      <div className='relative cursor-pointer' onClick={() => setIsCartOpen((prev) => !prev)}>
        <Image src='/cart.png' alt=' ' width={22} height={22} className='cursor-pointer'
           />
        <div className='absolute -top-4 -right-4 w-6 h-6 bg-lama rounded-full text-[#fff] text-sm items-center justify-center flex'>2</div>
      </div>
      {isCartOpen && (
        <CartModal />
      )}
