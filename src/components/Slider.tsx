            ...
            style={{ transform: `translateX(-${current * 100}vw)`}}>
                {slides.map(slide => (
                    <div className={`${slide.bg} w-screen h-full flex flex-col gap-16 xl:flex-row`} key={slide.id}>
                        {/* TEXT CONTAINER  */}
                        <div className='h-1/2 xl:w-1/2 flex items-center justify-center gap-8 flex-col 2xl:gap-12 text-center xl:h-full'>
                            <h2 className="text-xl lg:text-2xl 2xl:text-5xl">{slide.description}</h2>
                            <h1 className="text-5xl lg:text-6xl 2xl:text-8xl font-semibold">{slide.title}</h1>
                            <Link href={slide.url}><button className="rounded-md bg-black text-white py-3 px-4">SUBMIT</button></Link>
                        </div>
                        {/* IMAGE CONTAINER  */}
                        <div className='h-1/2 xl:w-1/2 relative xl:h-full'>
                            <Image src={slide.img} alt="yo" fill sizes="100%" className=" object-contain " />
                        </div>
                    </div>
                ))}
            </div>
